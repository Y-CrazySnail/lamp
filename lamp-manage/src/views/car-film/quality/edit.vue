...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      if ("string" == typeof this.form.workDate) {
        let year =
          Number(this.form.workDate.substring(0, 4)) +
          Number(this.form.guaranteePeriod);
        this.form.validityDate = year + this.form.workDate.substring(4);
      } else {
      ...
      this.$store
        .dispatch("car_film_quality/edit", this.form)
        .then((response) => {

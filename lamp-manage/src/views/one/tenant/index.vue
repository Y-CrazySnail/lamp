          ...
          >
            ...
            <el-col :span="6">
              <el-form-item label="客户手机:">
                <el-input
                  v-model="queryParam.tenantPhone"
                  clearable
                  placeholder="客户手机"
                />
              </el-form-item>
            ...
            <el-col :span="6">
              <el-form-item label="客户邮箱:">
                <el-input
                  v-model="queryParam.tenantEmail"
                  clearable
                  placeholder="客户邮箱"
                />
              </el-form-item>
...
import Add from "@/views/one/tenant/add";
import Edit from "@/views/one/tenant/edit";
export default {
  components: {
    "v-table": Table,
    "v-edit": Edit,
  ...
  methods: {
    fetchData(current, size) {
      this.$store
        .dispatch("one_tenant/page", {
          current: current,
          size: size,
          tenantName: this.queryParam.tenantName,
          tenantPhone: this.queryParam.tenantPhone,
          tenantEmail: this.queryParam.tenantEmail
        })
        .then(response => {
    ...
    edit(row) {
      this.$store
        .dispatch("one_tenant/get", { id: row.id })
        .then(response => {
    ...
    remove(row) {
      this.$store
        .dispatch("one_tenant/remove", { id: row.id })
        .then(response => {

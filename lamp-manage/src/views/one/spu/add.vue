<template>
  <div>
    <el-form ref="form" :model="form" label-width="120px">
      ...
      <vuedraggable v-model="form.spuAttributeList" :animation="400">
        <el-form-item
          :label="'属性' + Number(index + 1)"
          v-for="(attribute, index) in form.spuAttributeList"
          :key="index"
        >
      ...
      <div style="width: 470px">
        <el-form-item label="展示图">
          <el-upload
            ...
            :file-list="form.spuShowImageList"
            :headers="headers"
      ...
      <div style="width: 470px">
        <el-form-item label="轮播图">
          <el-upload
            action="http://localhost:8111/one/manage/spu/upload"
            :file-list="form.spuSwiperImageList"
            drag
            :headers="headers"
      ...
      <div>
        <el-form-item label="轮播图顺序">
          <vuedraggable
            v-model="form.spuSwiperImageList"
            style="display: flex"
            :animation="400"
          >
            <div v-for="(file, index) in form.spuSwiperImageList" :key="index">
              <img
      ...
      <div style="width: 470px">
        <el-form-item label="详情图">
          <el-upload
            action="http://localhost:8111/one/manage/spu/upload"
            :file-list="form.spuDetailImageList"
            drag
            :headers="headers"
      ...
      <div>
        <el-form-item label="详情图顺序">
          <vuedraggable
            v-model="form.spuDetailImageList"
            style="display: flex"
            :animation="400"
          >
            <div v-for="(file, index) in form.spuDetailImageList" :key="index">
              <img
                style="width: 150px; height: 150px; margin: 5px 5px 5px 5px"
                :src="file.url"
              />
...
export default {
  ...
  computed: {
    tenantList() {
      ...
      if (tenantList && tenantList.length > 0) {
        this.form.tenantId = tenantList[0].id;
      }
      return tenantList || [];
    },
    storeList() {
      ...
      if (storeList && storeList.length > 0) {
        this.form.storeId = storeList[0].id;
      }
      return storeList || [];
    },
    categoryList() {
      ...
      if (categoryList && categoryList.length > 0) {
        this.form.categoryId = categoryList[0].id;
      }
      return categoryList || [];
    },
  },
  methods: {
    onSubmit() {
      this.form.spuAttribute = JSON.stringify(this.form.spuAttributeList);
      this.form.spuSwiperImage = this.form.spuSwiperImageList
        .map((image) => image.url)
        .join(",");
      this.form.spuDetailImage = this.form.spuDetailImageList
        .map((image) => image.url)
        .join(",");
      this.form.spuShowImage = this.form.spuShowImageList
        .map((image) => image.url)
        .join(",");
      this.$store
        .dispatch("one_spu/save", this.form)
        .then((response) => {
    ...
    showHandleSuccess(response, file, fileList) {
      this.form.spuShowImageList = fileList.map((file) => {
        if (file.response) {
    ...
    swiperHandleSuccess(response, file, fileList) {
      this.form.spuSwiperImageList = fileList.map((file) => {
        if (file.response) {
          file.url = file.response;
        }
        return file;
    ...
    detailHandleSuccess(response, file, fileList) {
      this.form.spuDetailImageList = fileList.map((file) => {
        if (file.response) {
          file.url = file.response;
        }
        return file;
    ...
    addAttribute() {
      this.form.spuAttributeList.push({ attributeKey: "", attributeValue: "" });
    },
    removeAttribute(index) {
      this.form.spuAttributeList.splice(index, 1);
    },
  },
  destroyed() {
    this.form = {};

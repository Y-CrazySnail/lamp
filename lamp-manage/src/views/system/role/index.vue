  ...
  <div class="app-container">
    ...
    >
      <v-add
        ...
        v-bind:add-dialog-visible.sync="addDialogVisible"
      />
    </el-dialog>
    <el-dialog
      class="snail_dialog"
...
import Add from "@/views/system/role/add";
import Edit from "@/views/system/role/edit";
export default {
  name: "Role",
  components: {
    "v-table": Table,
  ...
  methods: {
    fetchData(current, size) {
      this.$store
        .dispatch("role/page", { current: current, size: size })
        .then((response) => {
    ...
    remove(row) {
      this.$store
        .dispatch("role/remove", { id: row.id })
        .then((response) => {
          this.fetchData();
        })
        .catch(() => {});

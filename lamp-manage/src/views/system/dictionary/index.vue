...
import Add from '@/views/system/dictionary/add'
import Edit from '@/views/system/dictionary/edit'
export default {
  name: 'Dictionary',
  components: {
    'v-table': Table,
  ...
  methods: {
    fetchData(current, size) {
      this.$store
        .dispatch('dictionary/page', { current: current, size: size })
        .then(response => {
    ...
    remove(row) {
      this.$store
        .dispatch('dictionary/remove', { id: row.id })
        .then(response => {
          this.fetchData()
        })
        .catch(() => {})

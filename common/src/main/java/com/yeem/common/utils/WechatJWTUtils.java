...
public class WechatJWTUtils {
    ...
    public static String generateJWT(String application, Long id, String openId) {
        ...
        log.info("generate jwt. application:{}, openId:{}, expires:{}, token:{}", application, openId, newTime, token);
        return token;
    }
    ...
    public static String parseJWTApplication(String token) {
        if (!validate(token)) {
            return null;
        } else {
            JWT jwt = JWT.of(token);
            jwt.setKey(KEY.getBytes(StandardCharsets.UTF_8));
            return String.valueOf(jwt.getPayload("application").toString());
        }
    }
    ...
    public static void main(String[] args) {
        System.out.println(generateJWT("zero-dev", 12345678L, "openIdTest"));
    }
}
